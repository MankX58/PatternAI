...
function TechSheetModal({ name, fullName, onClose, editorUrl, Design }) {
...
function PatternCard({ name, Design, viewMode, onSelect }) {
  ...
  return (
    <div className="pattern-card-link" onClick={() => onSelect({ name, fullName, editorUrl, Design })} style={{cursor: 'pointer'}}>
...
function App() {
  ...
  const filteredDesigns = Object.entries(designs).filter(([name, Design]) => {
    let fullName = name;
    try {
      const p = new Design();
    ...
    <div className="app-container">
      ...
      <main className="gallery-container">
        {filteredDesigns.length > 0 ? (
          filteredDesigns.map(([name, Design]) => (
            <PatternCard key={name} name={name} Design={Design} viewMode={viewMode} onSelect={setSelectedPattern} />
          ))
        ) : (
